...
class NetworkRequests {
    ...
    ///
    ///output URLRequest
    func setUrlRequest(urlString: String, method: HTTPMethod, params: [String: Any], header: [String: String]) -> URLRequest {
        //        guard (reachability?.connection != .unavailable) == true else {
        //
        //
        ...
        if method != .get {
            ...
            if  !params.isEmpty{
                ...
                if let urlToReturn = urlComponents.url {
                    urlRequest.url = URL(string: urlToReturn.absoluteString.trimmingCharacters(in: .whitespaces).replacingOccurrences(of: "%20", with: "", options: .literal, range: nil))
                }
            } else {
